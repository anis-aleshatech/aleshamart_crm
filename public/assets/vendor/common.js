...
function permissionsCampaign(tables,status){
	...
	if(data=="")
	{
		...
		return false;
			...
			var hrefdata ="permissionscampaign?approve_val="+data+"&&tablename="+tables+"&&status="+status;
			window.location.href=hrefdata;
	}
}


function permissionsTransaction(status){
	//alert(status);
	var summeCode=document.getElementsByName("summe_code[]");
	...
	for(var i=0; i < summeCode.length; i++){
		...
		{
			data[j]=summeCode[i].value;
			//datac[j]=customerid[i].value;
			//dataa[j]=amounts[i].value;
			j++;
	...
	if(data=="")
	{
		...
		return false;
			...
			//var hrefdata ="permissionstransaction?approve_val="+data+"&&custid="+datac+"&&amount="+dataa+"&&status="+status;
			var hrefdata ="permissionstransaction?approve_val="+data+"&&status="+status;
			window.location.href=hrefdata;
	}
}
